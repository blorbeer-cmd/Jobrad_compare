...
/**
 * BULLS adapter (bulls.de).
 * BULLS is a German e-bike brand (Stuttgart) with a strong JobRad presence.
 * Their online shop uses a custom React/Next.js storefront with JSON-LD schema.
 */
export class BullsAdapter extends BaseAdapter {
  readonly name = "BULLS";
  readonly cacheTtlMs = 6 * 60 * 60 * 1000;

  private baseUrl = "https://www.bulls.de";
  private searchUrls = [
    "/fahrraeder/e-bikes/",
    ...
    "/fahrraeder/e-city/",
    "/fahrraeder/e-cargo/",
    "/fahrraeder/trekking-crossbikes/",
  ...
  async fetchBikes(): Promise<Bike[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        console.error(`[BULLS] Error fetching ${path}:`, result.reason);
  ...
  protected parseListing(html: string, categoryPath: string): Bike[] {
    ...
    const jsonLdBikes = this.parseJsonLdProducts($, categoryPath, "BULLS");
    if (jsonLdBikes.length > 0) return jsonLdBikes;

    // Fallback: BULLS product card selectors
    const cards = $(
