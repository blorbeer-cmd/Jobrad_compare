...
/**
 * Pegasus adapter (pegasus.de).
 * Pegasus is a German bike brand (Stuttgart, part of Derby Cycle / Accell Group,
 * same parent as Kalkhoff) and a confirmed JobRad partner. They offer a wide
 * range of e-bikes sold via their own online shop and retail partners.
 * Their storefront uses a custom React app with JSON-LD product schema.
 */
export class PegasusAdapter extends BaseAdapter {
  readonly name = "Pegasus";
  readonly cacheTtlMs = 6 * 60 * 60 * 1000;

  private baseUrl = "https://www.pegasus.de";
  private searchUrls = [
    "/bikes/e-bikes/",
    "/bikes/e-trekking/",
    "/bikes/e-city/",
  ...
  async fetchBikes(): Promise<Bike[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        console.error(`[Pegasus] Error fetching ${path}:`, result.reason);
  ...
  protected parseListing(html: string, categoryPath: string): Bike[] {
    ...
    const jsonLdBikes = this.parseJsonLdProducts($, categoryPath, "Pegasus");
    if (jsonLdBikes.length > 0) return jsonLdBikes;

    // Fallback: Pegasus product card selectors
    const cards = $(
