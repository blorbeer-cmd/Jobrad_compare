...
/**
 * Scott Sports adapter (scott-sports.com/de).
 * Scott is a major Swiss sports brand with a strong German bike market presence
 * and a confirmed JobRad partnership. Their storefront uses a custom Next.js
 * app with SSR and JSON-LD product schema on category pages.
 */
export class ScottAdapter extends BaseAdapter {
  readonly name = "Scott";
  readonly cacheTtlMs = 6 * 60 * 60 * 1000;

  private baseUrl = "https://www.scott-sports.com";
  private searchUrls = [
    "/de/bikes/e-bikes/",
    "/de/bikes/e-mountainbike/",
    "/de/bikes/e-trekking-e-city/",
    "/de/bikes/mountainbike/",
    "/de/bikes/road/",
    "/de/bikes/gravel/",
    "/de/bikes/kids/",
  ];
  ...
  async fetchBikes(): Promise<Bike[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        console.error(`[Scott] Error fetching ${path}:`, result.reason);
  ...
  protected parseListing(html: string, categoryPath: string): Bike[] {
    ...
    const jsonLdBikes = this.parseJsonLdProducts($, categoryPath, "Scott");
    if (jsonLdBikes.length > 0) return jsonLdBikes;

    // Fallback: Scott product card selectors
    const cards = $(
