...
export class LuckyBikeAdapter extends BaseAdapter {
  ...
  private searchUrls = ["/e-bikes/","/fahrraeder/trekkingfahrraeder/","/fahrraeder/cityraeder-urbanraeder/","/fahrraeder/mountainbikes/","/fahrraeder/rennraeder/","/fahrraeder/gravel-crossraeder/"];
  ...
  async fetchBikes(): Promise<Bike[]> {
    ...
    for (const path of this.searchUrls) {
      ...
      } catch (error) { console.error(`[LuckyBike] Error fetching ${path}:`, error); }
    }
    return allBikes;
  }
  ...
  private parseListing(html: string, categoryPath: string): Bike[] {
    ...
    $(".product--box, .product-box, [data-product-id]").each((_, el) => {
      try {
        const $el = $(el);
        const name = $el.find(".product--title, .product-name, .product-title a").first().text().trim() || $el.find("a[title]").first().attr("title")?.trim();
        if (!name) return;
        const price = this.parsePrice($el.find(".product--price, .price--default, .product-price").first().text().trim());
        if (!price) return;
