...
/**
 * Haibike adapter (haibike.com).
 * Haibike is a leading German e-bike brand (part of Winora Group, Schweinfurt)
 * and a confirmed JobRad partner. Their storefront uses a custom React/Next.js
 * app with SSR and JSON-LD Product schema. All Haibike models are e-bikes.
 */
export class HaibikeAdapter extends BaseAdapter {
  readonly name = "Haibike";
  readonly cacheTtlMs = 6 * 60 * 60 * 1000;

  private baseUrl = "https://www.haibike.com";
  private searchUrls = [
    "/de/bikes/e-mountainbike/",
    "/de/bikes/e-trekking-e-city/",
    "/de/bikes/e-road/",
    "/de/bikes/hardtail/",
    "/de/bikes/fullsuspension/",
  ];
  ...
  async fetchBikes(): Promise<Bike[]> {
    ...
    for (let i = 0; i < results.length; i++) {
      ...
      if (result.status === "fulfilled") {
        ...
        console.error(`[Haibike] Error fetching ${path}:`, result.reason);
  ...
  protected parseListing(html: string, categoryPath: string): Bike[] {
    ...
    const jsonLdBikes = this.parseJsonLdProducts($, categoryPath, "Haibike");
    if (jsonLdBikes.length > 0) return jsonLdBikes;

    // Fallback: Haibike custom React product cards
    const cards = $(
      ".product-card, .bike-card, [data-testid='product-card'], " +
      "[class*='ProductCard'], [class*='BikeCard'], [class*='product-item']"
    );
    ...
    cards.each((_, el) => {
      try {
        ...
        const listPriceText = $el.find("del, s, [class*='old-price']").first().text().trim();
